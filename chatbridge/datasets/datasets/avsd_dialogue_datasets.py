...
import torch
from chatbridge.datasets.datasets.dialogue_datasets import (
    DialogueDataset,
    DialogueEvalDataset,
)
...
class AVSDDialEvalDataset(DialogueEvalDataset):
    ...
    def __getitem__(self, index):
        ...
        vname = os.path.join(self.vis_root, ann["image_id"]+'.mp4')
        aname = os.path.join(self.aud_root, ann["image_id"]+'.wav')

        video = self.vis_processor(vname)
        audio = self.aud_processor(aname)
        caption = self.text_processor(ann['caption'])
        dialogue = ""
        for qa in ann['dialog']:
            ques, ans = qa['question'], qa['answer']
            dialogue += f"###Human: {ques}###Assistant: {ans}"
        question = ann['question']
        dialogue += f"###Human: {question}###Assistant:"
