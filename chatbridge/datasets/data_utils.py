...
from chatbridge.datasets.datasets.base_dataset import ConcatDataset
...
def load_video(video_path, n_frms=MAX_INT, height=-1, width=-1, sampling="uniform"):
    ...
    if isinstance(vr_batch, torch.Tensor):
        frms = vr_batch.permute(3, 0, 1, 2).float() # (C, T, H, W)
    else:
        frms = torch.tensor(vr_batch.asnumpy()).permute(3, 0, 1, 2).float() # (C, T, H, W)
